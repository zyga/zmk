...
clean::
	$(call Silent.Say2,RM,$(addprefix man/,$(ZMK.manPages)))
...
$(addprefix check-,$(tests)): check-%:
	$(call Silent.Say2,MAKE-TEST,$(TESTDIR))
ifeq ($(ZMK.IsOutOfTreeBuild),yes)
	$(Silent.Command)mkdir -p $(TESTDIR)
	$(Silent.Command)$(strip $(MAKE)	--warn-undefined-variables \
